...
async function detailToRecord(item, credentials, listStatus, perpTickers) {
  ...
  return {
    ...
    SettlementCurrency: String(quote).replace(/\.PERP$/i, "").toUpperCase(),
    RawTotalProfit: firstNumber(data.totalProfit, data.total_profit),
    RawRealizedProfit: firstNumber(data.totalRealizedProfit),
    RawFundingFee: firstNumber(data.totalFundingFee, data.fundingFeePayment, data.fundingFee, data.funding_fee),
    RawFee: firstNumber(data.totalFee, data.fee, data.feeQuote, data.quoteFee),
    RawProfitWithdrawn: firstNumber(data.profitWithdrawn, data.profitWithdrawnU, data.profit_withdrawn, data.profitExited),
    RawProfitReinvest: firstNumber(data.profitReinvest, data.profit_reinvest),
    RawProfitReduce: firstNumber(data.profitReduce, data.profit_reduce),
    GridProfit24h: profit24h,
    Fee: scaleUsdt(firstNumber(data.totalFee, data.fee, data.feeQuote, data.quoteFee), coinMargined ? conversionPrice : null),
